...
i. Linear Regression
- It is a  supervised learning and used for Regression
...
- linear regression me ek line banta h and Agar data point line ke paas hai → model ne us data point ko sahi predict kiya hai (kam error).
...
a.Linearity:- The relationship between inputs (X) and the output (Y) is a straight line.
b. Errors are independent of each other:- Agar model ne ek observation (data point) ke liye galti ki, to us galti ka doosre observation (data point) ki galti par koi effect nahi hona chahiye.”
📊 Example:
...
- It is a supervised machine learning and used for  classification
